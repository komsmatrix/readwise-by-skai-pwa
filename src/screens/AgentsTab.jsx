...
export default function AgentsTab({ savedPass }) {
  ...
  const [view,        setView]        = useState('dashboard') // dashboard | create | sales
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setCreateStatus('error'); setCreateError(data.error || 'Failed to create agent')
      }
    } catch (err) {
      setCreateStatus('error'); setCreateError('Network error — ' + err.message)
    }
  }
  ...
  const topAgent     = agents.sort((a,b) => (b.totalSales||0) - (a.totalSales||0))[0]
  ...
  if (view === 'create') {
    return (
      <div style={s.section}>
        ...
        <p style={s.desc}>Add an agent — the system generates their unique referral code automatically.</p>
        ...
        {createStatus === 'success' && createResult && (
          <div style={s.successBox} className="animate-in">
            ...
            <p style={{ margin:'4px 0', fontSize:12, color:'var(--text-muted)', lineHeight:1.6 }}>
              Send this code to {createResult.name.split(' ')[0]} via Messenger. Customers get ₱20 off when they use it. You pay ₱50 commission per sale.
            </p>
  ...
  if (view === 'sales' && selectedAgent) {
    return (
      <div style={s.section}>
        ...
        {salesLoading ? (
          <div style={{ textAlign:'center', padding:40, color:'var(--text-muted)' }}>Loading sales...</div>
        ) : agentSales.length === 0 ? (
  ...
  return (
    <div style={s.section}>
      ...
      {/* Top agent */}
      {topAgent && topAgent.totalSales > 0 && (
        <div style={s.topAgentBox}>
          <p style={s.topAgentLabel}>🏆 Top Agent</p>
          <p style={s.topAgentName}>{topAgent.name}</p>
          <p style={s.topAgentStats}>{topAgent.totalSales} sales · ₱{(topAgent.totalEarned||0).toFixed(0)} earned · Code: <span style={{ color:'var(--accent)', fontFamily:'monospace' }}>{topAgent.code}</span></p>
        </div>
      )}
      ...
      {payStatus && Object.keys(payStatus).map(id => payStatus[id]?.success && (
        <div key={id} style={s.paidConfirm} className="animate-in">
          ✅ Paid ₱{payStatus[id].amount?.toFixed(0)} for {payStatus[id].count} sale{payStatus[id].count !== 1 ? 's' : ''} — agent notified by email
        </div>
      ))}
      ...
      {loading ? (
        ...
        <div style={s.agentList}>
          {[...agents].sort((a,b) => (b.totalSales||0) - (a.totalSales||0)).map((agent) => (
...
const s = {
  ...
  topAgentBox   : { background:'rgba(201,169,110,0.08)', border:'1px solid rgba(201,169,110,0.2)', borderRadius:'var(--radius-md)', padding:'12px 14px' },
  topAgentLabel : { margin:'0 0 4px', fontSize:11, color:'var(--accent)', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.06em' },
  topAgentName  : { margin:'0 0 2px', fontSize:15, fontWeight:600, color:'var(--text-primary)' },
  topAgentStats : { margin:0, fontSize:12, color:'var(--text-muted)' },
